...
from .motion_models.botsort_kalman import BotKalman
from .motion_models.ocsort_kalman import OCSORTKalman
from .motion_models.sort_kalman import SORTKalman
...
class Tracklet_w_velocity(Tracklet):
    ...
    def __init__(self,
        ...
        self.alpha_fixed_emb = 0.95  # default param
...
class Tracklet_w_velocity_four_corner(Tracklet):
    """Tracklet class with four corner points velocity and previous confidence,
    for hybrid sort and tracktrack."""
    ...
    def __init__(self,
        ...
        # score threshold to limit the range of kalman-predicted score and observation score
        self.score_thresh = score_thresh
        self.delta_t = delta_t

        self.age = 0  # mark the age

        # reid features, for tracktrack
        self.features = deque([], maxlen=feat_history)  # all features
        self.smooth_feat = None  # EMA feature
        self.curr_feat = None  # current feature
        if feat is not None:
    ...
    def speed_direction(point1, point2):
        """In order to jointly calculating the four corner velocity, parse
        point coordinate as input.
    ...
    def update(self, new_track, frame_id):
        ...
        if self.last_observation.sum() >= 0:  # exists previous observation
            ...
            for dt in range(1, self.delta_t + 1):  # from new to old
                if self.age - dt in self.observations:
                    ...
                    self.velocity_tl += self.speed_direction([previous_box[0], previous_box[1]],
                                                             [new_tlbr[0], new_tlbr[1]])
                    self.velocity_tr += self.speed_direction([previous_box[2], previous_box[1]],
                                                             [new_tlbr[2], new_tlbr[1]])
                    self.velocity_bl += self.speed_direction([previous_box[0], previous_box[3]],
                                                             [new_tlbr[0], new_tlbr[3]])
                    self.velocity_br += self.speed_direction([previous_box[2], previous_box[3]],
                                                             [new_tlbr[2], new_tlbr[3]])
        ...
        if self.enable_state_new:
            self.state = TrackState.Tracked if len(
                self.observations.keys()) >= self.state_tracked_min_len else TrackState.New
        else:
            self.state = TrackState.Tracked
...
class Tracklet_w_bbox_buffer(Tracklet):
    ...
    def __init__(self, tlwh, score, category, motion='byte'):
        ...
        self.origin_bbox_buffer = deque(
        )  # a deque store the original bbox(tlwh) from t - self.n to t, where t is the last time detected
        ...
        # motion state, s^{t + \delta} = o^t + (\delta / n) * \sum_{i=t-n+1}^t(o^i - o^{i-1}) = \
        # o^t + (\delta / n) * (o^t - o^{t - n})
    ...
    def get_buffer_bbox(self, level=1, bbox=None):
        ...
        if bbox is None:
            buffer_bbox = self._tlwh + np.array([
                -b * self._tlwh[2], -b * self._tlwh[3], 2 * b * self._tlwh[2], 2 * b * self._tlwh[3]])
        else:
    ...
    def predict(self):
        ...
        motion_state = self.origin_bbox_buffer[-1] + \
                    (self.time_since_update / len(self.origin_bbox_buffer)) * \
                          (self.origin_bbox_buffer[-1] - self.origin_bbox_buffer[0])
...
class Tracklet_w_UCMC(Tracklet):
    ...
    def tlwh_to_ground(self, tlwh=None):
        """Key function, map tlwh in camera plane to world coordinate
        ground."""
        ...
        sigma = np.array([[sigma_xc * sigma_xc, 0], [0, sigma_yc * sigma_yc]])
    ...
    def cal_maha_distance(self, det_ground_xy, det_sigma_ground_xy):
        """cal a mahalanobis dist between a track and det (Eq.
