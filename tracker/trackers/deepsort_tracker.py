...
from .matching import iou_distance, linear_assignment, nearest_embedding_distance, chi2inv95, matching_cascade
...
class DeepSortTracker(BaseTracker):
    ...
    def gated_metric(self, tracks, dets):
        """get cost matrix, firstly calculate apperence cost, then filter by
        Kalman state.

        tracks: List[STrack]
        dets: List[STrack]
        """
        appearance_dist = nearest_embedding_distance(tracks=tracks, detections=dets, metric='cosine')
    ...
    def gate_cost_matrix(self,
                         ...
                         tracks,
                         dets,
                         max_appearance_thresh=0.15,
                         gated_cost=1e5,
                         only_position=False):
        """
        gate cost matrix by calculating the Kalman state distance and constrained by
        ...
        cost_matrix[cost_matrix > max_appearance_thresh] = gated_cost
