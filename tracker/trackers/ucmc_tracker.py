...
class UCMCTracker(BaseTracker):
    ...
    def __init__(self, args, frame_rate=30):
        ...
        # storing the intrinsic and extrinsic param of camera
        ...
        z0 = 0  # the mapped z axis value in world coordinate, by default the objects are on the ground
        # so z0 = 0
    ...
    def _read_cam_param(self, ):
        """read the camera param, borrowed from
    ...
    def update(self, output_results, img, ori_img):
        ...
        if len(dets) > 0:
            """Detections."""
            detections = [
                Tracklet_w_UCMC(tlwh, s, cate, motion=self.motion) for (tlwh, s, cate) in zip(dets, scores_keep, cates)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            """Detections."""
            detections_second = [
                Tracklet_w_UCMC(tlwh, s, cate, motion=self.motion)
