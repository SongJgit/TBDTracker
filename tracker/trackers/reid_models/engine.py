...
import cv2
import numpy as np
from .OSNet import osnet_ibn_x1_0, osnet_x0_25, osnet_x0_75, osnet_x1_0, osnet_x0_5
...
def load_reid_model(reid_model, reid_model_path=None, device=None, trt=False, crop_size=[128, 64]):
    ...
    if reid_model not in REID_MODEL_DICT.keys():
        ...
        if trt:
            ...
            if not reid_model_path.endswith('.engine'):
                ...
                trt_converter = TensorRTConverter(model,
                                                  input_shape=[3, *crop_size],
                                                  ckpt_path=reid_model_path,
                                                  min_opt_max_batch=[1, 8, 32],
                                                  device=device,
                                                  load_ckpt=False)
...
def crop_and_resize(bboxes,
    ...
    crops = torch.empty((num_crops, 3, sz[1], sz[0]), dtype=torch.float, device=device)
...
def load_checkpoint(fpath):
    """loads checkpoint copy from https://github.com/KaiyangZhou/deep-person-
    reid."""
    if fpath is None:
...
def load_pretrained_weights(model, weight_path):
    ...
    if len(matched_layers) == 0:
        logger.warning('The pretrained weights "{}" cannot be loaded, '
