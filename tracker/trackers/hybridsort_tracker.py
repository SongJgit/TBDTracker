...
class HybridSortTracker(BaseTracker):
    ...
    def update(self, output_results, img, ori_img):
        ...
        velocities = np.array([trk.get_velocity() for trk in tracklet_pool])  # (N, 4, 2)
        ...
        matches, u_track, u_detection = association_weak_cues(tracklets=tracklet_pool,
        ...
        dists = hm_iou_distance(r_tracked_tracklets, detections_second) - score_distance(
        ...
        dists = hm_iou_distance(
            atracks=[t.last_observation[:4] for t in r_tracked_tracklets],  # parse bbox directly
