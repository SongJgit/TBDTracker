"""Measurement functions, Assignment and matching functions, Distance fusion
functions."""
...
def hm_iou_distance(atracks, btracks, return_original_iou=False):
    ...
    atlbrs_ = np.expand_dims(atlbrs, axis=1)  # (M, 4) -> (M, 1, 4) to apply broadcast mechanism
...
def score_distance(atracks, btracks):
    ...
    return score_diff_batch(det_scores=np.ascontiguousarray(bscores), track_scores=np.ascontiguousarray(ascores))


def nearest_embedding_distance(tracks, detections, metric='cosine'):
    """different from embedding distance, this func calculate the nearest
    distance among all track history features and detections.
...
def angle_distance(det_tlbrs, previous_obs, velocities, mode='center'):
    """calculate the angle between tracklet historical motion and the direction
    similarity of tracklet/detections i.e., this func measures the motion
    trend.
...
def greedy_assignment_cascade(dists, thresh):
    """iteratively record the best match used in TrackTrack.
...
def observation_centric_association(tracklets,
                                    detections,
                                    velocities,
                                    previous_obs,
                                    vdc_weight=0.05,
...
def observation_centric_association_w_reid(tracklets,
                                           detections,
                                           velocities,
                                           previous_obs,
                                           vdc_weight=0.05,
...
def association_weak_cues(tracklets,
                          detections,
                          velocities,
                          previous_obs,
                          score_diff_weight=1.0,
                          vdc_weight=0.05,
                          iou_threshold=0.25):
    """observation centric association with four corner point velocity,
    confidence score and HM IoU, for Hybrid Sort.
    ...
    det_tlbrs = np.array([det.tlbr for det in detections])
    det_scores = np.array([det.score for det in detections])
    # Note that the kalman-predicted score is used in first round association
...
def iterative_assignment(tracklets,
                         dets,
                         dets_second,
                         dets_delete,
                         velocities,
                         ...
                         reduce_step=0.05,
                         with_reid=True):
    """Used in TrackTrack association strategy in TPA module.
...
def bbox_ious(atlbrs, btlbrs, use_torchvision=False):
    """the function replace of the cython_bbox.bbox_overlaps.bbox_ious due to
...
def speed_direction_batch(dets, tracks, mode='center'):
    """helper func of observation_centric_association (OC Sort) and
    association_weak_cues (Hybrid Sort)
...
def cal_cosine_distance(mat1, mat2):
    """simple func to calculate cosine distance between 2 matrixs used in
...
def compute_aw_max_metric(embed_cost, w_association_emb, bottom=0.5):
    """observation centric association, with velocity and reid feature, for
    Deep OC Sort helper func of observation_centric_association_w_reid.
    ...
    for idx in range(embed_cost.shape[0]):
        ...
        if embed_cost[idx, inds[0]] == 0:
            ...
            row_weight = 1 - max((embed_cost[idx, inds[1]] / embed_cost[idx, inds[0]]) - bottom, 0) / (1 - bottom)
    ...
    for idj in range(embed_cost.shape[1]):
        ...
        if embed_cost[inds[0], idj] == 0:
            ...
            col_weight = 1 - max((embed_cost[inds[1], idj] / embed_cost[inds[0], idj]) - bottom, 0) / (1 - bottom)
