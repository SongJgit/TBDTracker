...
class OCSortTracker(BaseTracker):
    ...
    def update(self, output_results, img, ori_img):
        ...
        velocities = np.array([trk.velocity if trk.velocity is not None else np.array((0, 0)) for trk in tracklet_pool])
        ...
        dists = 1. - ious(
            atlbrs=[t.last_observation[:4] for t in r_tracked_tracklets],  # parse bbox directly
