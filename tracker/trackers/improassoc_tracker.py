...
from .matching import iou_distance, linear_assignment, d_iou_distance, embedding_distance
...
class ImproAssocTracker(BaseTracker):
    ...
    def __init__(
            ...
            motion_format: Dict[str, Any] | str | None = 'improassoc',
            track_buffer: int = 35,
        ...
        # some hyper params
        self.lambda_ = lambda_  # weight in Eq. 4
        self.proximity_thresh = proximity_thresh  # IoU thresh in Eq. 4, _o_min
        self.overlap_thresh = overlap_thresh  # Iou thresh in occlusion aware initialization, _o_max
        self.match_thresh, self.second_match_thresh = match_thresh, second_match_thresh  # empirically set in paper
    ...
    def update(self, output_results, img, ori_img):
        ...
        if self.reid_cfg is not None:
            ...
            dists = self.lambda_ * (1.0 - d_iou_dists) + (1. - self.lambda_) * emb_dists
        ...
        beta = self.match_thresh / self.second_match_thresh
        dists_concat = np.hstack([dists, beta * dists_second])
        detections_all = detections + detections_second

        # solve assignment
        matches, u_track, u_detection = linear_assignment(dists_concat, thresh=self.match_thresh)

        for itracked, idet in matches:
            track = tracklet_pool[itracked]
            det = detections_all[idet]
            if track.state == TrackState.Tracked:
                track.update(detections_all[idet], self.frame_id)
        ...
        # check each row, if any iou > self.overlap_thresh, the remain detection is removed
        if iou_matrix.size != 0:
            for col in range(num_of_detections_remain):
                if np.max(iou_matrix[:, col]) > self.overlap_thresh:
