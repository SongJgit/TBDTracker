...
class TensorRTConverter(object):
    ...
    def export(self):
        ...
        profile.set_shape(self.INPUT_NAME, min=self.min_input_shape, opt=self.opt_input_shape, max=self.max_input_shape)
...
class TensorRTInference(object):
    ...
    def __init__(self, engine_path, min_opt_max_batch=[1, 1, 1], use_fp16=False, device=torch.device('cpu')):
        ...
        self.load_engine()
        logger.info('load tensor rt engine done')
