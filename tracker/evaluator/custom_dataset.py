...
from trackeval import _timing, Evaluator
...
class CustomDataset(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        elif (self.config['TRACKERS_TO_EVAL'] is not None) and (len(self.config['TRACKER_DISPLAY_NAMES']) == len(
                self.tracker_list)):
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    ...
                    if not os.path.isfile(curr_file):
                        print('Tracker file not found: ' + curr_file)
                        raise TrackEvalException('Tracker file not found: ' + tracker + '/' + self.tracker_sub_fol +
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if is_gt:
            if self.filter_ignored_region:
                crowd_ignore_filter = {
                    self.config['CROWD_IGNORE_REGION']['col_idx']: self.config['CROWD_IGNORE_REGION']['class_id']}
            else:
        ...
        #     else:
        #         text = 'Tracking'
        #     raise TrackEvalException(text + ' data contains the following invalid timesteps in seq %s: ' % seq +
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                ...
                raw_data['dets'][t] = np.atleast_2d(
                    time_data[:, self.col_idx_map['bbox_start']:self.col_idx_map['bbox_end'] + 1])
                ...
                if is_gt:
                    ...
                    if self.zero_mark:
                        gt_extras_dict.update({
                            'zero_marked':
                            np.atleast_1d(time_data[:, self.col_idx_map['score']].astype(int))})

                    if self.filter_truncation:
                        gt_extras_dict.update({
                            'truncation':
                            np.atleast_1d(time_data[:, self.col_idx_map['truncation']].astype(int))})

                    if self.filter_occlusion:
                        gt_extras_dict.update({
                            'occlusion':
                            np.atleast_1d(time_data[:, self.col_idx_map['occlusion']].astype(int))})
            ...
            if is_gt:
                ...
                if time_key in ignore_data.keys():
                    ...
                    raw_data['gt_crowd_ignore_regions'][t] = np.atleast_2d(
                        time_ignore[:, self.col_idx_map['bbox_start']:self.col_idx_map['bbox_end'] + 1])
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if self.filter_ignored_region:
                ...
                intersection_with_ignore_region = self._calculate_box_ious(unmatched_tracker_dets,
